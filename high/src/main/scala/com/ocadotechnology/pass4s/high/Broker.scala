          ...
          source.capability match {
            ...
            case _                        =>
              throw new UnsupportedOperationException(
                s"Broker with capabilities [${Tag[P1]} with ${Tag[P2]}] doesn't support capability ${source.capability} from $source"
              )
          }
        ...
        override def sender[R >: P1 & P2]: Sender[F, Message[R]] =
          Sender.routed { message =>
            message.destination.capability match {
              ...
              case _                        =>
                throw new UnsupportedOperationException(
                  s"Broker with capabilities [${Tag[P1]} with ${Tag[P2]}] doesn't support capability ${message.destination.capability} from $message"
                )
            }
          }
      }.pure[F]
