...
/**
  * A runnable consumer of A.
  * To start one, you need a function that will handle messages of type A and return effects in F.
  *
  * For transactional processing, see [[Consumer#consumeCommit]]
  * or high-level utilities like MessageProcessor in the `extra` module.
  */
trait Consumer[F[_], +A] extends ((A => F[Unit]) => F[Unit]) with Serializable { self =>
  ...
  /**
    * Starts the consumer, passing every message through the processing function `f`.
    * Think of it like of an `evalMap` on [[Stream]] or `use` on [[cats.effect.Resource]].
    *
    * For consumers built from an infinite-stream Connector (which every connector you ever use is going to be), this never terminates.
    * For consumers like Consumer.one or Consumer.finite, this will succeed when all the messages have completed.
    *
    * There doesn't need to be any error handling in consumers in general,
    * but Consumer.sequential (used in any consumer built from a Connector) handles errors and uses them to rollback messages.
    */
  def consume(f: A => F[Unit]): F[Unit]

  /**
    * Starts the consumer, but allows the processing function `f` to be in a different effect than that of the consumer's.
    * A `commit` function also needs to be passed - it will be used after every message.
    */
  ...
  /**
    * Uses the given sender as the processing function.
    * Could be useful for creating simple message forwarders
    * between queues, brokers or even distinct platforms.
    *
    * Both [[Consumer]] and [[Sender]] are functions though,
    * so you could just call `apply(sender)` or `consume(sender)` directly.
    */
...
object Consumer extends ConsumerInstances {
  ...
  /**
    * A consumer that waits for processing of one message (and finalization of its resources), then takes another message from the stream of resources.
    * Failures that happen in the message handler are caught and used to rollback the message being processed.
    *
  ...
  /**
    * A consumer that can handle more than 1 message at once.
    * Failures that happen in the message handler are caught and used to rollback the message being processed.
    *
    * Restarts of the stream aren't handled here - this should be taken care of by the Connector implementation.
    *
    * Keep in mind that processing of message may end up faster that the processing of the previous message.
    * This strategy is not recommended when using FIFO queues.
    */
  ...
  /**
    * TODO: this is actually unused for now until we figure out how to properly handle failure and retries in SQS
    * A consumer that processes batches of messages in parallel. Messages within a batch are processed sequentially.
    * Failures that happen in the message handler are caught and used to rollback the message being processed.
    *
    * Restarts of the stream aren't handled here - this should be taken care of by the Connector implementation.
    *
    * This strategy should be safe for FIFO messages iff the broker ensures that messages in the batch are in FIFO order and that
    * messages with the same group id won't be seen in other batches before all the messages in the batch are committed. SQS is one example
    * of such message broker.
    */
  ...
  /**
    * [[cats.tagless.InvariantK]] instance for Consumer.
    * The effect appears in both covariant and contravariant positions,
    * so we can't get anything stronger like [[cats.tagless.FunctorK]].
    */
  ...
  /**
    * FunctorFilter instance for Consumer - allows ignoring certain messages,
    * so they never reach the processing function passed afterwards.
    */
  ...
  implicit final class ConsumerOps[F[_], A](private val self: Consumer[F, A]) extends AnyVal {
    ...
    /**
      * Adds an additional layer of processing before the message is sent to the handler (Kleisli composition before the handler).
      * Errors are reported normally and have the same impact on the consumer's process
    ...
    /**
      * For every message, executes provided finalization action that takes the original message as an input.
      * Useful for performing cleanup after the transaction has been completed successfuly.
      * Note that this behaves the same way finalizers would do. The earliest added action is executed the last
      * due to the nature of function composition.
      */
    def afterEach(f: A => F[Unit])(implicit F: FlatMap[F]): Consumer[F, A] =
      use => self.consume(msg => use(msg) >> f(msg))

    /**
      * For every message, creates an artificial consumer that only handles that one message, and runs it through the given function.
      * This follows [[Consumer#flatMap]] semantics, so while the consumer of `B` is busy processing, no further messages will be received by `self`.
      */
    ...
    /**
      * Merges the two consumers by returning one which will run them concurrently, with the same handler.
      * No synchronization between underlying consumers is involved - they will run completely independently.
      */
...
sealed trait ConsumerInstances extends ConsumerInstances1 {
  ...
  /**
    ...
    *
    * Defer is required for stack safety, because we can't guarantee it ourselves for any effect.
    * If you can implement a stack-safe tailRecM, hats off to you.
    */
  implicit def monad[F[_]: Defer]: Monad[Consumer[F, *]] =
    new Monad[Consumer[F, *]] with StackSafeMonad[Consumer[F, *]] with ConsumerApply[F] {
      ...
      /**
        ...
        *
        * Processing a message in the outer consumer will last as long as it takes to deplete the inner consumer.
        * If the inner consumer runs forever, the outer consumer will never see another message.
        */
  ...
  /**
    * Sequential semigroup for Consumer.
    * Consumers combined with this will run one after another (first the LHS will be depleted, then the RHS).
    */
  ...
  /**
    * Parallel semigroup for Consumer.
    * Consumers combined with this will run with the same processing function, concurrently.
    */
...
sealed trait ConsumerInstances1 extends ConsumerInstances0 {
  ...
  /**
    * Apply instance for Consumer. Most likely you'll never care about using it.
    * For an explanation on how it works, it's preferred to look at flatMap in [[Consumer.monad]] first.
    */
